...
namespace VirtualGrasp.Scripts
{
    ...
    public class VG_Recorder : MonoBehaviour
    {
        ...
        [Tooltip("Path to save the new Recording asset in. E.g. Assets/Recordings/new-recording.")]
        ...
        [Tooltip("Replay from in-memory recording without loading from a replay file. Requires having recorded with this checked first.")]
        public bool m_replayFromMemory = false;

        [Tooltip("Avatar to replay a specific interaction with. Has to be the avatar with Replay checked.")]
        ...
        [Tooltip("Avatar's hand to replay a specific interaction with.")]
        public VG_HandSide m_side = VG_HandSide.LEFT;
        [Tooltip("Specific interaction to replay (see console output for available numbers).")]
        ...
        {
            if (m_replayFromMemory) // in memory record and replay
            {
                if (!m_isRecording)
                {
                    VG_Controller.StartRecording();
                    m_isRecording = true;
                    Debug.Log("Started VG sensor recording.");
                }
                else
                {
                    ...
                    Debug.Log("Stopped VG sensor recording. Saved in memory");
                }
            }
            else // record to save permanently
            {
                if (!m_isRecording)
                {
                    if (m_newRecordingPath == null)
                    {
                        Debug.LogWarning("Cannot start VG sensor recording. new recording property not set.");
                    }
                    else
                    {
                        VG_Controller.StartRecording();
                ...
                else
                {
                    ...
                    if (m_newRecordingPath == null)
                    {
                        Debug.LogWarning("Cannot save VG sensor recording. new recording property not set.");
                    }
                    else
                    {
        ...
        private void CheckStopReplay()
        {
            ...
            foreach (int replayingAvatarInstanceID in GetReplayAvatars())
            {
                if (!VG_Controller.IsReplaying(replayingAvatarInstanceID, VG_HandSide.LEFT) && !VG_Controller.IsReplaying(replayingAvatarInstanceID, VG_HandSide.RIGHT))
                {
                    m_isReplaying = false;
                    m_replayID++;
        ...
        private bool PrepareReplay()
        {
            ...
            if (m_replayFromMemory)
            {
                if (m_tempRecording == null)
                {
                    VG_Debug.LogWarning("No recording in memory. Record something first");
                    return false;
                }
                else
                {
                    res = VG_Controller.LoadRecording(m_tempRecording);
                }
            }
            else
            {
            ...
            {
                VG_Debug.LogError("Loading recording failed");
                m_isReplaying = false;
                return false;
            }
        }
